...
export default function KBO(){
	...
	useEffect(()=>{
		Axios.get("https://stackemsports.onrender.com/api/players/kbo").then((response) => {
	...
	*/
	...
	let user_limit_kbo = 50;//5 * 5 //user_limit should be # of players x max available stats by a player
	
	
	if(window.matchMedia("(pointer: coarse)").matches) {
		// touchscreen is main input (ie.phone?)
		user_limit_kbo = 9; //5 is # of categories
			...
			<div class = "flip-card-container">
				{listOfUsers.filter((user)=> {
					if(search != ''){
					  ...
					  if(index  >=  user_limit_kbo){/*easier than putting if statement around player card return statement*/
						  return;
					  }
